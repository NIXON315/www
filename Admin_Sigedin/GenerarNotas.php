...
while ($row1 = $result1->fetch(PDO::FETCH_ASSOC)) {
    ...
    while ($row2 = $result2->fetch(PDO::FETCH_ASSOC)) {
        ...
        , `AnswerEva_QuesEvaId`
    FROM
        `sistema-escolar`.`EvaSys_AnswerEva` WHERE  `AnswerEva_UserNameEvaluated` = '$UserNameEvaluated' AND `AnswerEva_UserNameEvaluator` = '$UserNameEvaluator' AND `AnswerEva_GuysQuesId` ='1' AND `AnswerEva_IdPeriod` ='44' AND `AnswerEva_QuesEvaId` = $QuesEvaId  AND `AnswerEva_QualifyNum` IS NOT NULL AND `AnswerEva_QualifyNum` != ''; ";
